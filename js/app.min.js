...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateDropdownHeights() {
    ...
    document.querySelectorAll('.dropdown-catalog-nav__categories').forEach(function (menu) {
      var px = menu.scrollHeight;
      var vw = px / docWidth * 100;
      menu.style.setProperty('--dropdown-height', "".concat(vw, "vw"));
    });
  }
  function changePositionFilterDetailed() {
    var catalogFilterDetailed = document.querySelector('.catalog-filter-detailed');
    var catalogFilterMain = document.querySelector('.catalog-filter__dropdown');
    var catalogFilterDropdown = document.querySelector('.catalog-filter-dropdown__top');
    if (catalogFilterDetailed && catalogFilterMain && catalogFilterDropdown) {
      if (window.innerWidth <= 1023) {
        setTimeout(function () {
          catalogFilterDropdown.after(catalogFilterDetailed);
          catalogFilterDetailed.classList.remove('catalog-filter__detailed');
          catalogFilterDetailed.classList.add('catalog-filter-dropdown__detailed');
        }, 50);
      } else {
        setTimeout(function () {
          catalogFilterMain.append(catalogFilterDetailed);
          catalogFilterDetailed.classList.add('catalog-filter__detailed');
          catalogFilterDetailed.classList.remove('catalog-filter-dropdown__detailed');
        }, 50);
      }
    }
  }
  smInfoTrigger();
  changePositionFilterDetailed();
  var mobileMax = 1023;
  var observer;
  function initObserver() {
    ...
    if (window.innerWidth <= mobileMax && 'IntersectionObserver' in window) {
      var options = {
        root: null,
      ...
      document.querySelectorAll('.catalog-card__btn').forEach(function (btn) {
        return observer.observe(btn);
      });
    } else {
      document.querySelectorAll('.catalog-card__btn').forEach(function (btn) {
        return btn.classList.remove('is--visible');
      });
    }
  }
  if (document.querySelector('.catalog-card__btn')) {
    initObserver();
  }
  function dropdownBasketOffset() {
    if (document.querySelector('.header')) {
      if (document.querySelector('.top-notify')) {
        if (document.querySelector('.top-notify').classList.contains('is--hidden')) {
          document.querySelector('.dropdown-basket').style.top = document.querySelector('.header').getBoundingClientRect().height + 'px';
        } else {
          document.querySelector('.dropdown-basket').style.top = document.querySelector('.header').getBoundingClientRect().height + document.querySelector('.top-notify').getBoundingClientRect().height + 'px';
        }
      } else {
        document.querySelector('.dropdown-basket').style.top = document.querySelector('.header').getBoundingClientRect().height + 'px';
      }
  ...
  if (document.querySelector('.dropdown-basket')) {
    dropdownBasketOffset();
  }
  ...
  (function () {
    ...
    selectBoxes.forEach(function (selectBox) {
      ...
      selectedEl.addEventListener('click', function (e) {
        e.stopPropagation();
        var wasOpen = container.classList.contains('is--active');
        closeAllSelects();
        if (!wasOpen) {
  ...
  // filter select accordeon
  var filterSelectTrigger = document.querySelectorAll('.filter-select__trigger');
  if (filterSelectTrigger) {
    filterSelectTrigger.forEach(function (item) {
      item.addEventListener('click', function (event) {
        event.preventDefault();
        if (!item.parentNode.classList.contains('is--open')) {
          slideDownQna(item.nextElementSibling);
  ...
  var catalogFilterTrigger = document.querySelectorAll('.catalog-filter-trigger');
  var catalogFilterClose = document.querySelectorAll('.catalog-filter-dropdown__close');
  if (catalogFilter && catalogFilterTrigger && catalogFilterClose) {
    catalogFilter.addEventListener('click', function (event) {
      if (event.target.classList.contains('catalog-filter-dropdown')) {
        event.preventDefault();
  ...
  // // quantity
  // const quantity = document.querySelectorAll('.c-quantity')

  // if (quantity) {
  //     quantity.forEach(item => {
  //         const value = item.querySelector('.c-quantity__value')
  //         const btnMinus = item.querySelector('.c-quantity__btn--minus')
  //         const btnPlus = item.querySelector('.c-quantity__btn--plus')

  //         let currentValue = parseInt(value.dataset.value) || 0;
  ...
  // basket dropdown
  var basketTriggerDropdown = document.querySelector('.header__btn--basket');
  var basketDropdown = document.querySelector('.dropdown-basket');
  function onDocClick(e) {
    if (!basketDropdown.contains(e.target) && !basketTriggerDropdown.contains(e.target)) {
      closeDropdown();
    }
  }
  var openDropdown = function openDropdown() {
    basketDropdown.classList.add('is--active');
    document.addEventListener('click', onDocClick);
  };
  var closeDropdown = function closeDropdown() {
    basketDropdown.classList.remove('is--active');
    document.removeEventListener('click', onDocClick);
  };
  if (basketTriggerDropdown) {
    basketTriggerDropdown.addEventListener('click', function (e) {
      if (!isTouchDevice) {
        var dropdownItems = basketDropdown.querySelectorAll('.dropdown-basket__item');
        if (dropdownItems) {
          if (dropdownItems.length > 0) {
            e.preventDefault();
            e.stopPropagation();
            if (basketDropdown.classList.contains('is--active')) {
              closeDropdown();
            } else {
              openDropdown();
            }
  ...
  if (basketDropdown) {
    basketDropdown.addEventListener('click', function (e) {
  ...
  if (recommendsSliders) {
    recommendsSliders.forEach(function (slider) {
      ...
      nextZone.addEventListener('mouseenter', function (e) {
  ...
  var coordOffsetBottom = 0;
  var coordOffsetTop = 0;
  ...
  window.addEventListener('load', updateDropdownHeights);
  window.addEventListener('scroll', function () {
    productScroll();
    if (document.getElementById('top-notify') && window.pageYOffset >= 30) {
      document.getElementById('top-notify').classList.add('is--hidden');
    }
    if (document.querySelector('.dropdown-basket')) {
      dropdownBasketOffset();
    }
