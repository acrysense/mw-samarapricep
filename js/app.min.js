...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // recommends slider
  var recommendsSlider = document.querySelectorAll('.recommends__slider');
  if (recommendsSlider) {
    recommendsSlider.forEach(function (slider) {
      ...
      var halfWidth = function halfWidth() {
        return slider.clientWidth / 2;
      };
      var myRecommendsSlider = new Swiper(swiper, {
        slidesPerView: 'auto',
        spaceBetween: 26,
        navigation: {
          prevEl: prevBtn,
      ...
      slider.addEventListener('mousemove', function (e) {
        ...
        var y = e.clientY - slider.getBoundingClientRect().top;

        // определяем, в какой половине курсор
        if (x < halfWidth()) {
          // левая — показываем «назад»
          prevBtn.style.display = 'block';
          nextBtn.style.display = 'none';
          prevBtn.style.transform = "translate(".concat(x, "px, ").concat(y, "px)");
        } else {
          // правая — показываем «дальше»
          nextBtn.style.display = 'block';
          prevBtn.style.display = 'none';
          nextBtn.style.transform = "translate(".concat(x, "px, ").concat(y, "px)");
        }
      });
      slider.addEventListener('mouseleave', function () {
        // скрыть при уходе мышки
